...
bool HAL_RB_Push(RingBuffer_t *rb, uint8_t byte) {
  ...
  rb->head = (uint16_t)((rb->head + 1) % RING_BUFFER_SIZE);
  rb->count++;
...
bool HAL_RB_Pop(RingBuffer_t *rb, uint8_t *byte) {
  ...
  if (rb->count > RING_BUFFER_SIZE || rb->tail >= RING_BUFFER_SIZE) {
    rb->count = 0;
    rb->head = 0;
    rb->tail = 0;
    return false;
...
void HAL_Inject_Interference(uint8_t *buffer, uint8_t length) {
  ...
  uint8_t target_range = (uint8_t)(length - 1);

  int byte_to_corrupt = rand() % target_range;
  int bit_to_flip = rand() % 8;

  buffer[byte_to_corrupt] ^= (uint8_t)(1 << bit_to_flip);
}
